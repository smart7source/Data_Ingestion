...
from utils.helpers import create_dir, log_exception_details
...
class DataQuality(object):
    def __init__(self, dq_id: int, rules: list[dict] = None, email_execution_report_to: str = None,
        ...
        print("This is the Job Id")
        print(self.job_id)
        print("This is the output location....")
        print(self.execution_reports_dir)
    ...
    def execute_unique_rule(self, rule: Rule):
        ...
        """
        print(f"Executing DQ Rule for {rule.name} on {rule.columns}")
    ...
    def execute_query_rule(self, rule: Rule):
        ...
        query = rule.query
        print(f"Executing DQ Rule for {rule.name} using query {rule.query}")
    ...
    def execute_rules(self, df: DataFrame) -> tuple[bool, str]:
        ...
        """
        print("Starting data quality rules executions..")
    ...
    def generate_report(self):
        ...
        """
        print(f"Preparing Data quality rules report for {self.dq_id}")
        table_header = ' '.join(list(
            map(lambda header: f"<th>{header}</th>", ["Spark Application Id", "DQ ID", "Rule ID", "Rule Name",
                                                      "Rule type", "Description", "Columns/Query", "Pass Count",
                                                      "Fail Count",
                                                      "Total Count"])))

        def rules_and_result(result: RuleExecutionResult):
            table_data = [self.app_id,
                          self.dq_id,
                          result.rule.rule_id,
                          result.rule.name,
    ...
    def write_report_to_html(self, file_name):
        ...
        """
        print(f"Writing data quality execution report to html file {self.execution_reports_dir}/{file_name}")
        try:
            ...
            print("Error writing report to html, skipping writing report", ex)
