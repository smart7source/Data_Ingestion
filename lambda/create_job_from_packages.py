...
def lambda_handler(event, context):
    ...
    print(source_data)
    job_name="job_"+job_datetime
    script_location = f"s3://chello/ingestion/dataIngestion.py"
    default_args = {
        "--extra-py-files": f"s3://chello/ingestion/utils.pyz,s3://chello/ingestion/data_load.pyz",
        "--TempDir": f"s3://chello/temporary/"
    }
    #TODO: See if you can chage the Scrupt location or the Path based on this....
    ...
    try:
        ...
        print("*******************************************************")
        job_metadata = glue.start_job_run(JobName=glue_job["Name"], Arguments = { '--raw_s3_path' : source_data})
